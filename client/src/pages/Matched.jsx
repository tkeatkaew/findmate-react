  ...
  useEffect(() => {
    ...
    if (!dataFetched) {
      const fetchData = async () => {
        setIsLoading(true);
        try {
          const [traitsResponse, matchesResponse] = await Promise.all([
            axios.get(`/personalitytraits/${user.id}`),
            axios.get("/matches", { params: { user_id: user.id } }),
          ]);

          const currentTraits = traitsResponse.data;
          setCurrentUserTraits(currentTraits);

          // Calculate similarity for each matched user
          const matchesWithSimilarity = matchesResponse.data.map((match) => ({
            ...match,
            similarity: calculateSimilarity(match, currentTraits),
          }));

          // Sort by similarity
          const sortedMatches = matchesWithSimilarity.sort(
